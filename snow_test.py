...
def load_csv_to_snowflake(file_path, table_name, stage_name):
    ...
    try:
        ...
        if not table_exists:
            ...
            # Get existing columns and compare with new columns
            existing_columns = get_existing_columns(cursor, table_name)
            new_columns = [col for col in df.columns if col.upper() not in existing_columns]
            
            if new_columns:
                # Alter table to add new columns
                alter_table_to_add_columns(cursor, table_name, new_columns)
